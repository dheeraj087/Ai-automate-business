...
function SideBar() {
  ...
  return (
    !isMobile && <>
      <aside className={`w-15 sticky left-0 top-15 border-r border-[#5F5F5F] transition-width text-white duration-500 ${cut &&
        "w-40"} font-['Oswald',"sans-serif"] relative h-[90vh] bg-[#1A1C20]`}>

        {!cut

          ?

          (<HiMenu size={30} className='text-[#b3b3b3] cursor-pointer ml-3 mt-2' onClick={() => setcut(!cut)} />)

          :
          (<HiBackspace className='text-[#b3b3b3] cursor-pointer ml-3 mt-2' onClick={() => setcut(!cut)} size={30} />)
        }
        {!cut
